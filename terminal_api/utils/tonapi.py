...
import base58
import requests
from pytoniq import BaseWallet, Wallet, WalletV4R2
...
class TonCenterApi:
    ...
    headers = {"X-Api-Key": _toncenter_key}
    ...
    def get_jetton_balance(cls, address: str, jetton_address: str):
        ...
        response = requests.get(
            cls.api_url + cls.jetton_wallets_endpoint, params=params
        )
        response.raise_for_status()
    ...
    def run_get_method(cls, address: str, method: str, stack: list):
        body = {"address": address, "method": method, "stack": stack}
    ...
    def get_transactions(cls, address: str, limit: int):
        ...
        response = requests.get(
            cls.api_url + cls.get_transactions_endpoint, params=params
        )
        response.raise_for_status()
...
if __name__ == "__main__":
    print(
        TonCenterApi.get_account_info(
            "EQDhOY5FrggXpkbyKPbW76zLfwhfUW7IXrsrOg9gBVUmHDSn"
        )
    )
    print(
        TonCenterApi.get_account_info(
